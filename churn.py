...
def clean_user(df):
    '''
    Clean the dataframe (user) in order to make it ready for the machine learning algorithm.
    '''

    df = df[df['last_login'].notnull()]
    # df['created_at'] = user_table['created_at'].map(lambda x: (x-np.datetime64('1970-01-01T00:00:00Z'))/np.timedelta64(1, 's'))
    ...
    #df['last_login'] = df[['last_login']==np.datetime]
    #df['last_login'] = df.apply(lambda x: datetime.utcfromtimestamp(x['last_login'].tolist()/1e9))
...
#
#
# def weight_edge(x, impression):
#     if any((df1['value'] ==x['value']) & (df1['name'] == x['name'])):
#         return 1
#     else: return 0


def find_follower(follower, edges):
    pass
...
# def pre_modeling(df, column_y, column_id):
#
#     y = df[column_y]
#     df.drop(column_y, axis=1,inplace = True)
#     user_id = df[column_id]
...
#     return X_train, X_test, y_train, y_test, column_id
...
if __name__=="__main__":
    ...
    user_table = create_churn(user_table, 14) #possibly modify create_churn using a user usage decrease.
    ...
    smox, smoy = smooting(X_train_best, y)
