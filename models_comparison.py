...
class ModelComparator(object):
    def __init__(self, models_params, X, y):
        ...
        self.X = X
        self.y = y
        self.X_train, self.X_test, self.y_train, self.y_test = cross_validation.train_test_split(X, y, test_size=0.3, random_state=0)
    ...
    def best_model(self, iter = 20):
        for model in self.models:
            rndGrid = RandomizedSearchCV(model_param.model, params, n_iter=n_iter)
